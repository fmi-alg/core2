...
namespace IntervalDescartesB {
  ...
  */
  ...
  template <typename POL, typename VEC1, typename VEC2>
    void filterIsolRoots(POL& P, int deg, 
			 ...
			 VEC1 &small, BigFloat B, VEC2 &Binomial,
			 bool isPos) {
    ...
    for(int i=0; i <= deg ; i++)
      slv[i] = BigFloat2(Q.coeff()[i], DOUBLE_PREC);

    int EPS_INIT = DOUBLE_PREC - 2;
    typedef IntvData<BigFloat, int, prec_t> IntvData;
    std::stack<IntvData> reinit;   
    IntvData ID(0, 1, 0, DOUBLE_PREC); 
    IntervalDescartesB::isolateRoots(slv, deg, ID,				     
				     vT, monotone, reinit, EPS_INIT);
    ...
    while(!reinit.empty()){
	ID = reinit.top(); reinit.pop();
	temp.push_back(BFInterval(ID.a, ID.b));
    }

    getInterval(temp, small, B, isPos);
  ...
  */
  template <typename T>
    ...
    IntervalDescartesB::filterIsolRoots(P, deg, v, checkMonotone, small, 
					B, Binomial, false);
    if(P.coeff()[0] == 0)
