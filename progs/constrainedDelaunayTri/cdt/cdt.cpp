...
HePointer   Mesh::getHullHalfedge(VPointer v,int sep_index,bool lowside)
{
   ...
   int   v_i;
   v_i=v->getIndex();
   //int   v_j;	// this variable is set, but not used? (Chee, Mar'2014)

   he=v->out_he;
   if   (he==NULL)   return   NULL;

   //v_j=he->v->getIndex(); // this variable is set, but not used? (Chee, Mar'2014)
...
HePointer Mesh::getHullHalfedgeNextTo(HePointer _he,VPointer v,int l,int sep_index,int r,bool lowside)
{
   ...
   int   v_i;
   v_i=v->getIndex();
   // int   v_j;	// Chee (Mar'2014): this variable is set but not used

   he=v->out_he;

   if   (he->next_he==v->out_he)   return NULL;

   // v_j=he->v->getIndex();  // Chee (Mar'2014): v_j is set but not used
...
void   Mesh::mergeSlabPair(SPointer   ls)
{
   ...
   //int      m,l,r;   // middle separator line's index (Chee: m is set but not used)
   int      l,r;   // middle separator line's index
   ...
   l=ls->l;
   // m=ls->r; 	// Chee (Mar'2014): m is set but not used
   r=rs->r;
...
void   olddisplay(){
   ...
   //display
   SPointer   tmps;
   while   (mesh.s_head->next!=NULL)
   {
      tmps=mesh.s_head;
      // one scan of list, merging each two consecutive slabs
      while(tmps!=NULL   && tmps->next!=NULL)
      {
         mesh.mergeSlabPair(tmps);
         // now the ts is already the merged slab
         tmps=tmps->next;
         mesh.display();
      }
...
//      glViewport(0,-hh+h/2,(GLsizei)w,hh+h/2);
      if   (ratio<h/w)
      {
         ...
         glViewport(0, (GLsizei)h/2-hh, w, (GLsizei)h/2+hh); /* Set Viewport */
      }
      else
      {
         GLsizei ww=(int)(h/ratio/2);
         glViewport((GLsizei)w/2-ww, 0, (GLsizei)w/2+ww, h); /* Set Viewport */
      }
...
int   main(int argc,char* argv[]){
   if   (argc   <2 || argc >3)
   {
      printf("Usage: ./cdt.exe source_file_name [destine_file_name]\n");
      return(-1);
   }
   else
   {
      int   value=mesh.import2OFFFile(argv[1]);
      ...
      if   (value==0)
      {
         ...
         glutCreateWindow("Before Triangluated");
         init();
...
//         glutMainLoop();
         ...
         if (argc==2)
         {
   //         writeResult(argv[1],NULL);
            mesh.export2OFFFile(argv[1],NULL);
         }
         else
         {
   //            writeResult(argv[1],argv[2]);
            mesh.export2OFFFile(argv[1],argv[2]);
         }
//         glutInit(&argc,argv);
//         glutInitDisplayMode(GLUT_SINGLE|GLUT_RGB);
         glutInitWindowSize(250,250);
         glutInitWindowPosition(400,100);
         glutCreateWindow("After Triangluated");
//         init();
