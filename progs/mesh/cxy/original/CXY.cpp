/* **************************************
   ...
   REMARKS:
	Chee (Jan'2010):
		...
		But we should implement the Level 2 version
		BigFloat2 should be used for intervals
		Also generalized and debugged the argument processing!
...
//typedef BigInt	myNT;
//typedef BigFloat	myNT;
typedef double  myNT;
typedef Polynomial<myNT>			PolyNT;
typedef std::vector<myNT>			VecNT;
typedef std::vector<Polynomial<myNT> >	VecPoly;
Curve<myNT> Bipolyno("x^2");
Curve<myNT> Differenticate_X;
Curve<myNT> Differenticate_Y;
vector<box*> Pic;
...
Point::Point(){
	...
	int Point::sign(){
		...
		ImplicitCurve tmp;
		value = tmp.value(x, y);
		value = Bipolyno.yPolynomial(x).eval(y);
		if (value<0)
			return -1;
		if (value>0)
			return 1;
...
// 
// =========================================================
Interval eval(BiPoly<myNT> Bi,Interval x, Interval y );
Interval evalpoly(Polynomial<myNT> po, Interval x);
...
int main(int argc, char* argv[])
{
	...
	case 2:
	   ...
	   if (offsetValue != 0) {
	        cout<<"\n============="<<endl;
		// fixed a bug 
		//OLD: Bipolyno.coeffX[0].setCoeff(0,offsetValue);
		//NEW:
		(Bipolyno.coeffX[0]).setCoeff(0,
			(Bipolyno.coeffX[0]).getCoeff(0)+offsetValue);
	        Bipolyno.dump("Input Curve with offset is");
	   }
	   cout<<"\n-------------"<<endl;
	   break;
	case 1:
