...
public:
  // Constructor
  MKPredicates(const CORE::Curve<DT> &fxy, 
	  const CORE::Curve<DT> &gxy, const DT &min_size, const DT &max_size,
...
//	  << sign(temp.getR()) << "]" << std::endl;
  ...
  // 1 if MK(B) holds
  // Juan:: this is not true if the jacobian condition is defined as above
...
//	std::cout<<"Box  " << x_range << " * " << y_range << " "<<std::endl;
	...
	CORE::BiPoly<DT> F, G, tmp1(fxy_), tmp2(gxy_), tmp3(fxy_), tmp4(gxy_);
...
*/	// END OF FILTER
	...
	if (!I_l.zero() && !I_r.zero() && (sign(I_l.getR())*sign(I_r.getR()) < 0))
    {
        P = G.fixY(y_l);
	    ...
	    if (!I_d.zero() && !I_u.zero() && (sign(I_d.getR())*sign(I_u.getR()) < 0))
            return 1;
    }
	/*
...
private:
  ...
  CORE::Curve<DT>			  jacobian;	// the determinant of jacobian_ matrix
