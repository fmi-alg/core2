...
public:
  ...
  BoxT() : generation_id(0), isLeaf(true), depth(1), pParent(NULL), priority(0), rB(0), flag_jc(false)
  {
      for (int i = 0; i < 4; ++i)
        {
            pChildren[i] = 0;
  ...
  BoxT(unsigned int gid,
       const CORE::IntervalT<NT> &x_range,
       const CORE::IntervalT<NT> &y_range) :
        generation_id(gid),
        x_range(x_range),
        y_range(y_range),
        isLeaf(true), depth(1), pParent(NULL), priority(0), rB(0), flag_jc(false)
  {
      for (int i = 0; i < 4; ++i)
        {
            pChildren[i] = 0;
  ...
  BoxT( const BoxT<NT>& other ):
      ...
      isLeaf(other.isLeaf), depth(other.depth), pParent(other.pParent), priority(other.priority), rB(other.rB), flag_jc(other.flag_jc)
  {
      for (int i = 0; i < 4; ++i)
      {
  ...
  */
  ...
  {
    CORE::IntervalT<NT> new_x = x_range.Expand(factor);
    CORE::IntervalT<NT> new_y = y_range.Expand(factor);
  ...
  // indicate whether jacobian condition holds on 3B,
  // it is default to be false
  // can be inherited to the children
  bool flag_jc;
...
// split when it is not too small
template<typename NT>
bool BoxT<NT>::Split(NT epsilon)
{
    ...
    children[0] = new BoxT<NT>(gen_id,CORE::IntervalT<NT>(x_start, x_mid), CORE::IntervalT<NT>(y_mid, y_end));
    children[1] = new BoxT<NT>(gen_id,CORE::IntervalT<NT>(x_mid, x_end), CORE::IntervalT<NT>(y_mid, y_end));
    children[2] = new BoxT<NT>(gen_id,CORE::IntervalT<NT>(x_mid, x_end), CORE::IntervalT<NT>(y_start, y_mid));
    children[3] = new BoxT<NT>(gen_id,CORE::IntervalT<NT>(x_start, x_mid), CORE::IntervalT<NT>(y_start, y_mid));
...
//    }
    ...
    for (int i = 0; i < 4; ++i)
    {
        ...
        children[i]->flag_jc = this->flag_jc;
