...
//////////////////////////////////////////////////////////////////////////////////
string egName("SoCG2016_non-cross-2links_demo1.eg");
...
int main(int argc, char* argv[]) {
    ...
    // Test if the build directory is non-cross-2-links. If so,
    // the path to the current working directory will
    // include /non-cross-2-links
    int indexOfDesiredDir = workingDir.rfind("/non-cross-2-links-qt/");
    if (indexOfDesiredDir != std::string::npos) {
        workingDir = workingDir.substr(0, indexOfDesiredDir + 5);

        // Set current working directory to non-cross-2-links
    ...
    // /non-cross-2-links-master instead of /non-cross-2-links
    if (!foundFiles &&
        (indexOfDesiredDir = workingDir.rfind("/non-cross-2-links-qt/")) != std::string::npos) {
        workingDir = workingDir.substr(0, indexOfDesiredDir + 12);

        // Set current working directory to /non-cross-2-links-master
        QDir::setCurrent(workingDir.c_str());

        foundFiles = true;
    }

    // Test if a build directory (/build-non-cross-2-links-...) was created. This directory
    // will reside in the same directory as /non-cross-2-links or /non-cross-2-links-qt
    if (!foundFiles &&
        (indexOfDesiredDir = workingDir.rfind("/build-non-cross-2-links-qt")) != std::string::npos) {
        QDir dir(workingDir.substr(0, indexOfDesiredDir).c_str());

        if (dir.exists("non-cross-2-links-qt/non-cross-2-links-qt.pro")) {                  // Test if /v2-links exists
            workingDir = workingDir.substr(0, indexOfDesiredDir) + "/non-cross-2-links-qt";

            // Set current working directory to non-cross-2-links
            QDir::setCurrent(workingDir.c_str());

            foundFiles = true;
        } else if (dir.exists("non-cross-2-links-qt/non-cross-2-links-qt.pro")) {  // Test if /non-cross-2-links-qt exists
            workingDir = workingDir.substr(0, indexOfDesiredDir) + "/non-cross-2-links-qt";
    ...
    // Neither /non-cross-2-links nor /non-cross-2-links-qt could be found
    if (!foundFiles) {
        ...
        "Make sure that the program is inside of a folder named \"non-cross-2-links-qt\".\n"
        "If it is not, rename the folder to \"non-cross-2-links-qt\" before running again.\n";
    }
