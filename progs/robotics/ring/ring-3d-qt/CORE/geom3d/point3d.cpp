...
void Point3d::separation_circle( Circle3d &cir, Point3d *& cir_p ) const {
  Point3d p_ = cir.toPlane().projection(Point3d(x, y, z));
  Vector v(p_-cir.P());
  double c = cir.R()*cir.R()/(Vector(p_-cir.P())*Vector(p_-cir.P()));

  double t1 = sqrt(c);
  double t2 = -sqrt(c);

  Point3d q1 = cir.P()+t1*v;
  Point3d q2 = cir.P()+t2*v;

  if(q1.distance(p_) < q2.distance(p_)){
    cir_p = new Point3d(q1);
  }
  else{
    cir_p = new Point3d(q2);
  }
}

Point3d* Point3d::separation_circle( Circle3d &cir ) const {
  Point3d p_ = cir.toPlane().projection(Point3d(x, y, z));
  Vector v(p_-cir.P());
  double c = cir.R()*cir.R()/(Vector(p_-cir.P())*Vector(p_-cir.P()));
  ...
  if(q1.distance(p_) < q2.distance(p_)){
    return new Point3d(q1);
  }
  else{
    return new Point3d(q2);
  }
}

double Point3d::separation_circleL( Circle3d &cir ) const {
  Point3d p_ = cir.toPlane().projection(Point3d(x, y, z));
  Vector v(p_-cir.P());
  double c = cir.R()*cir.R()/(Vector(p_-cir.P())*Vector(p_-cir.P()));
  ...
  double d1 = q1.distance(p_);
  double d2 = q2.distance(p_);

  if(d1 < d2){
    return cir.P().distance(q1);
  }
  else{
    return cir.P().distance(q2);
  }
}
