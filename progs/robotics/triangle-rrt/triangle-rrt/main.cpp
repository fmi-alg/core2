...
CFG start(250,360,0,false);
CFG goal(260,80, 70,false);
int l1,l2;
float scale=1.0;
...
float env_RR = 1.0 / (180);						// Rotational RESOLUTION (deg / 180)
...
int parseArg(int argc, char** argv)
{
	...
	if(argc >= 10) {
		...
		start.x = atof(argv[2]);
		start.y = atof(argv[3]);
		start.theta = atof(argv[4]);

		goal.x = atof(argv[6]);
		goal.y = atof(argv[7]);
		goal.theta = atof(argv[8]);
...
int main(int argc, char** argv)
{
	...
	else
	{
		...
		// 3 end points of the robot, relevant coordinates to the center of the triangle
		vector<Vector2d> triangle_robot = {Vector2d(-10, 0),Vector2d(10, 0), Vector2d(0,10)};

		// init robot
		planner->getRobot().init(triangle_robot, start);
