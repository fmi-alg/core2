    ...
    if( !m_DisalbeMouseControl ){
        ...
        else if( m_PressedButton == GLUT_MIDDLE_BUTTON )
        {
            if(!m_2D)
            {
                ...
                m_deltaDis[0] = ((GLfloat)(x - m_StartX))*0.250*zoom_diff; //((m_CameraPos[0]>5)?m_CameraPos[0]:5) * ((GLfloat)(x - m_StartX))/20.0;
                m_deltaDis[1] = ((GLfloat)(y - m_StartY))*0.250*zoom_diff; //((m_CameraPos[1]>5)?m_CameraPos[1]:5) * ((GLfloat)(y - m_StartY))/20.0;
            }
        }
