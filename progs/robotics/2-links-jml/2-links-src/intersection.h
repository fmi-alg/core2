...
inline char SegSegInt( const double a[2], const double b[2], const double c[2], const double d[2], double p[2] )
{
   ...
   if( fabs(num_s)<TINYNUMBER ) num_s=0;
   else if( fabs(num_s-denom)<TINYNUMBER ) num_s=denom;
   ...
   if( fabs(num_t)<TINYNUMBER ) num_t=0;
   else if( fabs(num_t-denom)<TINYNUMBER) num_t=denom;
