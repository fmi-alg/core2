...
int main(int argc, char* argv[]) {
    ...
    // Test if the build directory is complex-general-qt. If so,
    // the path to the current working directory will
    // include /complex-general-qt
    unsigned long indexOfDesiredDir = workingDir.rfind("/complex-general-qt/");
    if (indexOfDesiredDir != std::string::npos) {
        workingDir = workingDir.substr(0, indexOfDesiredDir + 5);

        // Set current working directory to complex-general-qt
    ...
    // /2-links-master instead of /complex-general-qt
    if (!foundFiles &&
        (indexOfDesiredDir = workingDir.rfind("/complex-general-qt/")) != std::string::npos) {
        workingDir = workingDir.substr(0, indexOfDesiredDir + 12);

        // Set current working directory to /complex-general-qt-master
        QDir::setCurrent(workingDir.c_str());

        foundFiles = true;
    }

    // Test if a build directory (/build-complex-general-qt-...) was created. This directory
    // will reside in the same directory as /complex-general-qt
    if (!foundFiles &&
        (indexOfDesiredDir = workingDir.rfind("/build-complex-general-qt")) != std::string::npos) {
        QDir dir(workingDir.substr(0, indexOfDesiredDir).c_str());

        if (dir.exists("complex-general-qt/complex-general-qt.pro")) {                  // Test if /complex-general-qt exists
            workingDir = workingDir.substr(0, indexOfDesiredDir) + "/complex-general-qt";
    ...
    // /complex-general-qt could not be found
    if (!foundFiles) {
        ...
        "Make sure that the program is inside of a folder named \"complex-general-qt\".\n";
    }
