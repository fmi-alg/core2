...
// copy constructor
Plane3d::Plane3d(const Plane3d& pl)
    : a(pl.A()), b(pl.B()), c(pl.C()), d(pl.displacement()), n(pl.normal()) {}

// plane with direction v passes through point p
Plane3d::Plane3d(const Point3d& p, const Vector& v)
    : a(v[0]), b(v[1]), c(v[2]), d(0.0), n(v), p0(p) {
...
Plane3d::Plane3d(const Point3d& p1, const Point3d& p2, const Point3d& p3) {
  ...
  p0.setX((p1.X() + p2.X() + p3.X()) / 3.0);
  p0.setY((p1.Y() + p2.Y() + p3.Y()) / 3.0);
  p0.setZ((p1.Z() + p2.Z() + p3.Z()) / 3.0);
}
...
bool Plane3d::contains(const Point3d& p) const { return apply(p) == 0; }
...
GeomObj* Plane3d::intersection(const Line3d& l) const {
  ...
  Point3d interPt =
      l.startPt() +
      (l.direction() * (-apply(l.startPt()) / (dotProduct(n, l.direction()))));
...
GeomObj* Plane3d::intersection(const Plane3d& pl) const {
  if (isParallel(pl)) {
    ...
    Vector l_direction = n.cross(pl.normal());  // direction of intersection
                                                // line
    ...
    if (p1 == p2) return new Line3d(p1, l_direction);
