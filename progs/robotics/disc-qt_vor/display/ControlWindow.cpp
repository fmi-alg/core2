...
ControlWindow::ControlWindow(QWidget* parent) :
  ...
  ui->doubleSpinBox_ax->setValue(alpha.X());
  ui->doubleSpinBox_ay->setValue(alpha.Y());
  ui->doubleSpinBox_bx->setValue(beta.X());
  ui->doubleSpinBox_by->setValue(beta.Y());
  ...
  switch(SearchType){
    ...
    case PriorityQ::GREEDY:
      ui->radioButton_greedy->setChecked(true);
      break;
    case PriorityQ::VORONOI:
      ui->radioButton_vor->setChecked(true);
      break;
    default:
      char error[] = "Search Type not recognized";
      if(exit)
        exit(SearchType, error);
      else
        ::exit(SearchType);
      break;
  }
...
void boxTraverse(Box* b) {
  ...
  for(int i=0; i<4; ++i){
    ...
    if(tmp != NULL &&
       d_mouseX >= tmp->mB.X()-tmp->width/2  && d_mouseX < tmp->mB.X()+tmp->width/2 &&
...
void ControlWindow::mousePressEvent(QMouseEvent *event) {
  ...
  if(d_record != NULL){
    ...
    sprintf(tmp_buff, "feature numbers corner %d wall %d vor corner %d wall %d\n",
  ...
  if (!run || !exit) {
    ...
    if(!missingArgs.size()) {
      if(strcmp(egPre, egCur) == 0) {
        ...
        alpha.setX(ui->doubleSpinBox_ax->value());
        alpha.setY(ui->doubleSpinBox_ay->value());
        beta.setX(ui->doubleSpinBox_bx->value());
        beta.setY(ui->doubleSpinBox_by->value());
        ...
        if(ui->radioButton_rand->isChecked())
          SearchType = PriorityQ::RANDOM;
        else if(ui->radioButton_bfs->isChecked())
          SearchType = PriorityQ::BFS;
        else if(ui->radioButton_greedy->isChecked())
          SearchType = PriorityQ::GREEDY;
        else if(ui->radioButton_vor->isChecked())
          SearchType = PriorityQ::VORONOI;
...
void ControlWindow::on_radioButton_rand_clicked(bool checked){
  ui->radioButton_rand->setChecked(checked);
}
void ControlWindow::on_radioButton_bfs_clicked(bool checked){
  ui->radioButton_bfs->setChecked(checked);
}
void ControlWindow::on_radioButton_greedy_clicked(bool checked){
  ui->radioButton_greedy->setChecked(checked);
}
void ControlWindow::on_radioButton_vor_clicked(bool checked){
  ui->radioButton_vor->setChecked(checked);
}
