...
int main(int argc, char* argv[]) {
    ...
    // include /thick-links
    int indexOfDesiredDir = workingDir.rfind("/thick-links-qt-windows/");
    ...
    if (!foundFiles &&
        (indexOfDesiredDir = workingDir.rfind("/thick-links-qt-windows/")) != (int)std::string::npos) {
    ...
    // will reside in the same directory as /thick-links or /thick-links-qt-windows
    if (!foundFiles &&
        (indexOfDesiredDir = workingDir.rfind("/build-thick-links-qt-windows")) != (int)std::string::npos) {
        QDir dir(workingDir.substr(0, indexOfDesiredDir).c_str());

        if (dir.exists("thick-links-qt-windows/thick-links-qt-windows.pro")) {                  // Test if /v2-links exists
            workingDir = workingDir.substr(0, indexOfDesiredDir) + "/thick-links-qt-windows";

            // Set current working directory to thick-links
            QDir::setCurrent(workingDir.c_str());

            foundFiles = true;
        } else if (dir.exists("thick-links-qt-windows/thick-links-qt-windows.pro")) {  // Test if /thick-links-qt-windows exists
            workingDir = workingDir.substr(0, indexOfDesiredDir) + "/thick-links-qt-windows";
    ...
    // Neither /thick-links nor /thick-links-qt-windows could be found
    if (!foundFiles) {
        ...
        "Make sure that the program is inside of a folder named \"thick-links-qt-windows\".\n"
        "If it is not, rename the folder to \"thick-links-qt-windows\" before running again.\n";
    }
